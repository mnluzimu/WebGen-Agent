...
def compute_step_level_grpo_advantage(token_level_rewards: List,
                                   response_mask: torch.Tensor,
                                   index: np.ndarray,
                                   epsilon: float = 1e-6):
    """
    ...
    (with only one scalar reward for each response).
    Args:
        token_level_rewards: List
            [[{"reward": int, "position" int}, {...}, ...], ...]
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    scores = copy.deepcopy(token_level_rewards)
    print(scores)
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) > 0:
                score_list = []
                for score in id2score[idx]:
                    for step_score in score:
                        score_list.append(step_score["reward"])
                print(f"prompt index: {idx}, score_list: {score_list}")
                if len(score_list) == 1:
                    id2mean[idx] = torch.tensor(0.0)
                    id2std[idx] = torch.tensor(1.0)
                else:
                    id2mean[idx] = torch.mean(torch.tensor(score_list, dtype=torch.float64))
                    id2std[idx] = torch.std(torch.tensor(score_list, dtype=torch.float64))
                print(f"prompt index: {idx}, mean: {id2mean[idx]}, std: {id2std[idx]}")
            else:
                raise ValueError(f"no score in prompt index: {idx}")

        for i in range(bsz):
            for step_score in scores[i]:
                step_score["reward"] = (step_score["reward"] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
        rewards = response_mask.clone().float()

        for i in range(bsz):
            pre = 0
            for step_score in scores[i]:
                if step_score["position"] == -1:
                    rewards[i, pre:] = rewards[i, pre:] * step_score["reward"]
                else:
                    rewards[i, pre:step_score["position"]] = rewards[i, pre:step_score["position"]] * step_score["reward"]
                    pre = step_score["position"]

    return rewards, rewards


def compute_cumulative_step_level_grpo_advantage(token_level_rewards: List,
                                   response_mask: torch.Tensor,
                                   index: np.ndarray,
                                   epsilon: float = 1e-6):
    """
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            for j in range(len(scores[i])):
                curr_adv = sum([step_score["reward"] for step_score in scores[i][j:]])
                if step_score["position"] == -1:
                    rewards[i, pre:] = rewards[i, pre:] * curr_adv
                else:
                    rewards[i, pre:step_score["position"]] = rewards[i, pre:step_score["position"]] * curr_adv
                    pre = step_score["position"]

    return rewards, rewards


def compute_grpo_max_advantage(token_level_rewards: List,
                                   response_mask: torch.Tensor,
                                   index: np.ndarray,
                                   epsilon: float = 1e-6):
    """
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    scores_lists = copy.deepcopy(token_level_rewards)
    print(scores_lists)
    print(len(scores_lists))
    scores = []
    for score_list in scores_lists:
        curr_score = max([step_score["reward"] for step_score in score_list]) if len(score_list) > 0 else 0.0
