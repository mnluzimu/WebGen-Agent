...
class AdvantageEstimator(str, Enum):
    ...
    GRPO = 'grpo'
    STEP_GRPO = 'step_grpo'
    CUM_STEP_GRPO = 'cumulative_step_grpo'
    GRPO_MAX = 'grpo_max'
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_step_level_grpo_advantage(
            token_level_rewards=data.non_tensor_batch['token_level_rewards'],
            response_mask=data.batch['response_mask'],
            index=data.non_tensor_batch['uid'])
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.CUM_STEP_GRPO:
        advantages, returns = core_algos.compute_cumulative_step_level_grpo_advantage(
            token_level_rewards=data.non_tensor_batch['token_level_rewards'],
            response_mask=data.batch['response_mask'],
            index=data.non_tensor_batch['uid'])
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.GRPO_MAX:
        advantages, returns = core_algos.compute_grpo_max_advantage(
            token_level_rewards=data.non_tensor_batch['token_level_rewards'],
            response_mask=data.batch['response_mask'],
            index=data.non_tensor_batch['uid'])
        data.batch['advantages'] = advantages
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
                ...
                AdvantageEstimator.GRPO, AdvantageEstimator.STEP_GRPO, AdvantageEstimator.CUM_STEP_GRPO, AdvantageEstimator.GRPO_MAX, AdvantageEstimator.REINFORCE_PLUS_PLUS, AdvantageEstimator.REMAX,
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.rollout.n_trajectories is not None:
            real_train_batch_size = config.data.train_batch_size * config.actor_rollout_ref.rollout.n_trajectories
        else:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            elif self.config.actor_rollout_ref.rollout.task_type == "webgen":
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = gen_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n_trajectories, interleave=True)
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if isinstance(reward_tensor_dict['all'], torch.Tensor):
                            batch.batch['token_level_scores'] = reward_tensor_dict['all']
                            for k, v in reward_tensor_dict.items():
                                if isinstance(v, torch.Tensor):
                                    batch.batch[k]=v
                        else:
                            # this is when reward is step-level [[{"reward": r, "position": p}], [...], ...](for webgen)
                            batch.non_tensor_batch['token_level_scores'] = reward_tensor_dict['all']
                            for k, v in reward_tensor_dict.items():
                                if isinstance(v, torch.Tensor):
                                    batch.batch[k]=v
                        ...
                        if isinstance(reward_tensor_dict['all'], torch.Tensor):
                            ...
                            batch.non_tensor_batch['token_level_rewards'] = batch.non_tensor_batch['token_level_scores']
