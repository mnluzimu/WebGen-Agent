    ...
    if "token_level_scores" not in batch.batch or "token_level_rewards" not in batch.batch:
        def compute_scores(scores):
            ...
            for score in scores:
                curr_scores = []
                for step in score:
                    curr_scores.append(step['reward'])
                scores_list.append(max(curr_scores) if len(curr_scores) > 0 else -1)
            return torch.tensor(scores_list, dtype=torch.float32)
        sequence_score = compute_scores(batch.non_tensor_batch['token_level_scores'])
        sequence_reward = compute_scores(batch.non_tensor_batch['token_level_rewards'])
    else:
