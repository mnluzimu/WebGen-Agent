...
def generate_summary(result, instruction, model):
    prompt = summary_prompt.format(gui_instruction=instruction, result=result)
    messages = [
        {"role": "system", "content": "You are a helpful agent."},
        {"role": "user", "content": prompt},
    ]
    output = vlm_generation(messages, model)
    return output


def get_webvoyager_feedback(idx, output_dir, instruction, url, vlm_model, model):
    task = {"web_name": idx, 
    "id": idx, 
    "ques": instruction, 
    "web": url}
    ...
    print(f"Running webvoyager test...")
    ...
    if messages[-1]["role"] == "assistant" and "YES" in messages[-1]["content"]:
        summary = generate_summary(result, instruction, model)
