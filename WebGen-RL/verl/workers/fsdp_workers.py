...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(timeout=timedelta(minutes=300))
        ...
        # normalize config
        if self._is_actor:
            if self.config.rollout.n_trajectories is not None:
                self.config.actor.ppo_mini_batch_size *= self.config.rollout.n_trajectories
            else:
